...
const tokenJson = require('../contracts/Lesson13/artifacts/MCSToken.json')
...
describe('MShop', function () {
    ...
    it('allow to buy', async function () {
        const tokenAmount = 3
        const txData = {value: tokenAmount, to: shop.address} // 1wei = 1 token
     ...
     it('allow to buy', async function () {
       const tx=await buyer.sendTransaction({value: 3, to: shop.address })
       await tx.wait();
